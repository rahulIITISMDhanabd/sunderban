...
// This replaces the previous script.js
...
async function loadShapefiles() {
    ...
    try {
        ...
        showError('Failed to load geographic data');
        showLoading(false);
    }
}
...
// Enhanced layer creation for proper polygon display
...
// Add village labels for better visibility
function addVillageLabels() {
    if (!geoData.villages || !showVillageLabels) return;
    
    const villageProp = getVillagePropertyName();
    
    villageLabelsLayer = L.layerGroup();
    
    geoData.villages.features.forEach(feature => {
        if (feature.geometry && feature.geometry.type === 'Polygon') {
            // Calculate centroid for label placement
            const coords = feature.geometry.coordinates[0];
            let centroid = getCentroid(coords);
            
            const villageName = feature.properties[villageProp] || 'Unknown';
            const cleanName = feature.properties.village_clean || villageName;
            
            // Only show labels for larger villages at lower zoom levels
            const marker = L.marker([centroid[1], centroid[0]], {
                icon: L.divIcon({
                    className: 'village-label',
                    html: `<span style="font-size: 10px; background: rgba(255,255,255,0.8); padding: 2px 4px; border-radius: 3px; border: 1px solid #ccc;">${cleanName}</span>`,
                    iconSize: [0, 0],
                    iconAnchor: [0, 0]
                })
            });
            
            villageLabelsLayer.addLayer(marker);
        }
    });
    
    // Add labels to map (initially hidden, shown at higher zoom levels)
    map.on('zoomend', function() {
        const zoom = map.getZoom();
        if (zoom >= 12) {
            if (!map.hasLayer(villageLabelsLayer)) {
                map.addLayer(villageLabelsLayer);
            }
        } else {
            if (map.hasLayer(villageLabelsLayer)) {
                map.removeLayer(villageLabelsLayer);
            }
        }
    });
}
...
// Rest of the functions remain the same but with enhanced village handling...
// (keeping the core functionality but improving village display)

// Sample data for fallback (simplified for space)
async function loadSampleData() {
    // This would contain sample data if the actual files aren't available
    console.log('Loading sample data...');
    // Implementation similar to previous version but with more villages
}
